  ...
  for(int i = grid->I_Min(); i <= grid->I_Max(); i++)
    for(int j = grid->J_Min(); j <= grid->J_Max(); j++)
      for(int k = grid->K_Min(); k <= grid->K_Max(); k++){
        ...
        f_neg_back = (T).5* ((*U_et)(i,  j,  k) - fabs((*U_et)(i,  j,  k  )) );
        f_pos_back = (T).5* ((*U_et)(i,  j,  k) + fabs((*U_et)(i,  j,  k  )) );
        f_neg_frnt = (T).5* ((*U_et)(i,  j-1,k) - fabs((*U_et)(i,  j-1,k  )) );
        f_pos_frnt = (T).5* ((*U_et)(i,  j-1,k) + fabs((*U_et)(i,  j-1,k  )) );
        f_neg_nrth = (T).5* ((*U_zt)(i,  j,  k) - fabs((*U_zt)(i,  j,  k  )) );
        f_pos_nrth = (T).5* ((*U_zt)(i,  j,  k) + fabs((*U_zt)(i,  j,  k  )) );
        f_neg_suth = (T).5* ((*U_zt)(i,  j,k-1) - fabs((*U_zt)(i,  j,  k-1)) );
        f_pos_suth = (T).5* ((*U_zt)(i,  j,k-1) + fabs((*U_zt)(i,  j,  k-1)) );
        ...
        a_back_u  = (T)-0.5 * (*U_et)(i,j,k)
          + (T).125 * (f_pos_back - (T)2*f_neg_back - f_neg_frnt);
        a_frnt_u  = (T).5   * (*U_et)(i,j-1,k)
          + (T).125 * (f_pos_back + (T)2*f_pos_frnt - f_neg_frnt);
        a_nrth_u  = (T)-.5  * (*U_zt)(i,j,k)
          + (T).125 * (f_pos_nrth - (T)2*f_neg_nrth - f_neg_suth);
        a_suth_u  = (T).5   * (*U_zt)(i,j,k-1)
        ...
        expression(i,j,k) +=a_east_u * (*U)(i+1,j,k) + a_west_u * (*U)(i-1,j,k) 
          + a_back_u * (*U)(i,j+1,k) + a_frnt_u * (*U)(i,j-1,k)
          + a_nrth_u * (*U)(i,j,k+1) + a_suth_u * (*U)(i,j,k-1)
          - a_cntr_u * (*U)(i,j,k)
          + (T).125 * (f_neg_east * (*U)(i+2,j,k) - f_pos_west  * (*U)(i-2,j,k)
              + f_neg_back * (*U)(i,j+2,k) - f_pos_frnt  * (*U)(i,j-2,k)
              + f_neg_nrth * (*U)(i,j,k+2) - f_pos_suth  * (*U)(i,j,k-2));
      }
} 
//*****************************************************************************
...
{  
  ...
  if(parameters->lid_velocity)
    for(int i = U_zt->I_Min(); i <= U_zt->I_Max(); i++)
      for(int j = U_zt->J_Min(); j <= U_zt->J_Max(); j++)
        (*U_zt)(i,j,upper_boundary[2]+1) = 
          (*grid->ZT_z)(i,j,upper_boundary[2]+1)
          * (*parameters->lid_velocity)(i,j).z;

  // Update flux on bottom boundary based on bed_velocity
  if(parameters->bed_velocity)
    for(int i = U_zt->I_Min(); i <= U_zt->I_Max(); i++)
      for(int j = U_zt->J_Min(); j <= U_zt->J_Max(); j++)
        (*U_zt)(i,j,lower_boundary[2]-1) = 
          (*grid->ZT_z)(i,j,lower_boundary[2]-1)
          * (*parameters->bed_velocity)(i,j).z;
}
//*****************************************************************************
