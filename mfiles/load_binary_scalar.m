...
function [P] = load_binary_scalar(directory, ts_start,delta_ts, npx,npy,npz, l_ni_h2,l_nj_h2,l_nk_h2)
    filename = [directory 'scalar.'];
    total_size_h2 = l_ni_h2 * l_nj_h2 * l_nk_h2;     % flat array_3d size w/ 1 halo cell
    
    int_sz = 4;
    dbl_sz = 8;
    
    str = sprintf('Reading Scalar...');  disp(str);
    for pk = 1:npz
        for pj = 1:npy
            for pi = 1:npx 
